...
export default function CardPreview({pokemon, loading = false}) {

    if (loading) return <CardPreviewLoading />
    
    return (
        <div className="rounded-lg overflow-hidden w-44 aspect-square relative shadow-[0px_1px_3px_1px_rgba(0,_0,_0,_0.2)]">
            <small className="absolute top-2 right-2 text-slate-500">#{pokemon.id.toString().padStart(3, 0)}</small>
            <div className="absolute top-0 bottom-0 right-0 left-0 text-center flex justify-center flex-col items-center z-10">
                <Image src={pokemon.sprites.front_default || pokemon.sprites.front_shiny || '/pokemon-skeleton.png'} width={120} height={120} alt={`Pokemon ${pokemon.name}`} />
